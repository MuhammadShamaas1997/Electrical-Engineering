...
Y=zeros(n,n);
for b=1:20
    Y=AddLine(Y,branch(b,1),branch(b,2),branch(b,3),branch(b,4),branch(b,5),branch(b,9));
end

Y=AddLine(Y,3,15,0,0.1,0,1);
Y=AddLine(Y,4,16,0.1,0.1,0,1);
...
for iteration=1:maxiter
    ...
    GenQ(3,1)=GenQ(3,1)+DeltaVoltage(n+1,1);
    GenQ(6,1)=GenQ(6,1)+DeltaVoltage(n+4,1);
    GenQ(8,1)=GenQ(8,1)+DeltaVoltage(n+6,1);
    LoadQ(n-1,1)=LoadQ(n-1,1)-DeltaVoltage(2*(n-1)-1,1);
end
