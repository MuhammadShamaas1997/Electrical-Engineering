...
% for ind=1:max+1
%         muap(ind)=10000*exp(-(abs(MMF(ind)-500))*(abs(MMF(ind)-500))/50000);
%         muan(ind)=-10000*exp(-(abs(MMF(ind)+500))*(abs(MMF(ind)+500))/50000);
% end
% 
% % hold on;
...
% subplot(2,1,1)
[mmf,emf]=dsort(ScopeData5.signals(1,3).values(200:2061),ScopeData5.signals(1,4).values(200:2061));
plot(mmf,emf);
ylabel('Generated EMF (V)');xlabel('Net MMF (rpm)');
% axis([400 2200 0 150]);
% subplot(2,1,6)
% plot(ScopeData5.time,ScopeData5.signals(1,5).values)
% xlabel('Time (s)');ylabel('Terminal Voltage (V)');axis([0 0.04 50 150]);
