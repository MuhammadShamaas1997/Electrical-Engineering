...
def load_SCOUT_permittivity(filename):#{{{
    ...
    f.seek(151);    datalength = np.fromfile(f, dtype=np.uint16, count=1)
...
for number, data_source in enumerate(data_sources):
    try:
        ...
        if len(data_sources)>5 and not (len(sys.argv)>2 and (sys.argv[2] == "--big")): plotlabel = "" ## disables legend if too long

        if eps!=None or (n!=None and k!=None):
            if eps==None: eps=(n+1j*k)**.5
            plt.subplot(3,1,1)
            ...
            plt.xlim(freq_range); plt.xscale('log'); plt.grid(True)
            plt.yscale('symlog')
        if n!=None:
            plt.subplot(3,1,2)
